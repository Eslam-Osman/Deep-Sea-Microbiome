...
# Random_Forest Analysis on microbiome
```{r}
...
df_final <- read_excel("Data/df_final.xlsx")
df= df_final
Meta=df[,1:28]
Mat=df[,29:8630]
...
print(RF_all_Seep)
#plot(RF_all_Seep)
#legend("right", colnames(RF_all_Seep$err.rate),col=1:7,cex=0.8,fill=1:4, horiz=F)# to add legend
...
All_Seep_im_top=All_Seep_im[1:25,]
head(All_Seep_im_top$features)
#write.csv(All_Seep_im_top$features, "Output/RF/All_Seep_im_top.csv")

```


##This is to check which taxa were acting as classifer for all variables.
```{r}
...
RF_Callo_site_im_top=All_Year_im[1:15,]
head(RF_Callo_site_im_top$features)
...
RF_Callo_Year_im_top=All_Year_im[1:15,]
head(RF_Callo_Year_im_top$features)
...
head(RF_Callo_Seep_im_top$features)
...
RF_para_Year_im_top=RF_para_Year_im[1:15,]
head(RF_para_Year_im_top$features)
...
Mat_wat_perc=Seawater_perc[,29:8630]
...
head(RF_wat_site_im_top$features)
...
head(RF_wat_Year_im_top$features)
...
head(RF_wat_Seep_im_top$features)
...
head(RF_sedi_site_im_top$features)
...
RF_sedi_Year_im_top=RF_sedi_Year_im[1:15,]
head(RF_sedi_Year_im_top$features)
...
head(RF_sedi_Seep_im_top$features)
